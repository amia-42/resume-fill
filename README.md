...
- 自定义字段：在任意固定区块（如“基本信息”）里补充 schema 没有的字段（如生源地、政治面貌），自行添加“字段名 + 字段值”，与标准字段一起参与 AI 字段映射和填充；AI 导入不会覆盖自定义字段
...
你也可以维护多份简历模板（例如分别面向测开、游戏开发、普通开发）。在侧边栏或简历配置页里切换、新建、复制、重命名、删除模板；填充前在“自动填充”页选择要使用的那一份。所有模板可一键导出为 JSON 备份，也可从 JSON 恢复。
